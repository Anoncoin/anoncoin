...
const uint32_t MAX_SCRIPT_ELEMENT_SIZE = 520; // bytes
const uint32_t MAX_OP_RETURN_RELAY = 80;      // bytes

//! Some v10 flags and definitions, greatly reduced for our needs...
/**
 ...
 * details.
 */
const uint32_t MANDATORY_SCRIPT_VERIFY_FLAGS = SCRIPT_VERIFY_P2SH;
/**
 * Standard script verification flags that standard transactions will comply
 * with. However scripts violating these flags may still be present in valid
 * blocks and we must accept those blocks.
 */
const uint32_t STANDARD_SCRIPT_VERIFY_FLAGS = SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC;
...
//        return false;
    ...
    assert( (flags & SCRIPT_VERIFY_LOW_S) == 0 );   //Anoncoin can not yet support this flag
    return true;
}
...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty())
    {
        ...
        // Anoncoin address tx, sender provides hash of pubkey, receiver provides signature and pubkey
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    // In order to use the old EvalScript, we need some details from the BaseSignatureChecker
    // EvalScript(stack, scriptSig, txTo, nIn, flags, nHashType)
    // txTo, nIn and nHashType
