...
public:
    ...
    void seenBlock(const std::vector<CTxMemPoolEntry>& entries, int nBlockHeight, const CFeeRate minRelayFee)
    {
        ...
        for (size_t i = 0; i < entriesByConfirmations.size(); i++)
        {
            ...
            {
                // Fees are stored and reported as ANC-per-kb:
...
void CTxMemPool::removeCoinbaseSpends(const CCoinsViewCache *pcoins, unsigned int nMemPoolHeight)
{
    ...
    for (std::map<uint256, CTxMemPoolEntry>::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin) {
            ...
            if (!coins || (coins->IsCoinBase() && static_cast<const int32_t>(nMemPoolHeight - coins->nHeight) < COINBASE_MATURITY)) {
...
CTxMemPool::WriteFeeEstimates(CAutoFile& fileout) const
{
    try {
        LOCK(cs);
        fileout << 90500; // version required to read: 0.9.05 or later
...
bool CCoinsViewMemPool::GetCoins(const uint256 &txid, CCoins &coins) const {
    ...
    CTransaction tx;
    bool fResult = mempool.lookup(txid, tx);
    if( fResult ) {
        // LogPrintf( "CCoinsViewMemPool::GetCoins() found %s in the mempool\n", txid.ToString() );
        coins = CCoins(tx, MEMPOOL_HEIGHT);
        return true;
    }
    fResult = base->GetCoins(txid, coins);
    bool fPruned = fResult ? coins.IsPruned() : false;
    // LogPrintf( "CCoinsViewMemPool::GetCoins() for %s called base->GetCoins(), which returned=%d, coins show IsPruned=%d\n", txid.ToString(), fResult, fPruned );
    return fResult && !fPruned;
}

bool CCoinsViewMemPool::HaveCoins(const uint256 &txid) const {
    bool fMempool = mempool.exists(txid);
    bool fHaveCoins = base->HaveCoins(txid);

    // LogPrintf( "CCoinsViewMemPool::HaveCoins() for %s Mempool=% base->HaveCoins() returned=%d\n", txid.ToString(), fMempool, fHaveCoins );
    return fMempool || fHaveCoins;
}
