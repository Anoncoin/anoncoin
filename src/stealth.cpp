// Copyright (c) 2017 The Anoncoin developers
...
const uint8_t stealth_version_byte = (!fTestNet) ? 0x17 : 0x53;
...
uint32_t AnoncoinChecksum(uint8_t* p, uint32_t nBytes) {
    if (!p || nBytes == 0)
        return 0;
...
void AppendChecksum(data_chunk& data) {
    uint32_t checksum = AnoncoinChecksum(&data[0], data.size());
...
bool VerifyChecksum(const data_chunk& data) {
    ...
    return AnoncoinChecksum((uint8_t*)&data[0], data.size()-4) == checksum;
}
