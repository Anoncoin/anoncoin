...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // This should be a good place to initialize I2Pd.
    i2p::api::StartI2P();

    if (GetBoolArg(I2P_SAM_GENERATE_DESTINATION_PARAM, false)) {
        auto keys = I2PSession::Instance().GenerateKeys ();
#ifdef ENABLE_QT
        uiInterface.ThreadSafeShowGeneratedI2PAddress(
            "Generated I2P address",
            keys.GetPublic ()->ToBase64 (), // pub
            keys.ToBase64 (), // priv
            I2PSession::Instance ().GetB32Address (),
            GetConfigFile().string()
        );
#else
    LogPrintf("Generated I2P address: dest=%s, priv=%s", keys.GetPublic ()->ToBase64(),keys.ToBase64() );
#endif
        return false;
    }
