...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/anoncoin.conf are parsed in qt/bitcoin.cpp's main()
...
{
    uiInterface.ThreadSafeMessageBox(str, _("Anoncoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("Anoncoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Anoncoin is probably already running."), GetDataDir().string().c_str()));
    ...
    // requested to kill anoncoin-qt during the last operation. If so, exit.
