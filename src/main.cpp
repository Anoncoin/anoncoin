...
{
    if (hashGenesisBlock == pindex->GetBlockHash())
        return true;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Anoncoin: Special short-term limits to avoid 10,000 BDB lock limit:
    ...
    // Check proof of work matches claimed amount
    if (!vtx.empty() && fCheckPOW && !CheckProofOfWork(GetPoWHash(), nBits))
...
{
    // Anoncoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool VerifyDB(int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = pindexBest; pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if ((nCheckLevel >= 1 && !block.CheckBlock(state))&&pindex->nHeight != 0)
