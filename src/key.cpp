...
public:
    ...
    bool Sign(const uint256 &hash, std::vector<unsigned char>& vchSig) {
        ...
        BIGNUM *sig_s = BN_CTX_get(ctx);
        BIGNUM *sig_r = BN_CTX_get(ctx);
        assert (BN_copy (sig_s, ps) != NULL);
        assert (BN_copy (sig_r, pr) != NULL);   // XXX not needed / not modified
        ...
        ECDSA_SIG_set0 (sig, sig_s, sig_r);
    ...
    bool SignCompact(const uint256 &hash, unsigned char *p64, int &rec) {
        ...
        if (nBitsR <= 256 && nBitsS <= 256) {
            ...
            assert(fOk);
            ECDSA_SIG_get0 (sig, &r, &s);   // XXX needed? don't think so
...
void CExtKey::SetMaster(const unsigned char *seed, unsigned int nSeedLen) {
    static const unsigned char hashkey[] = {'A','n','o','n','c','o','i','n',' ','s','e','e','d'};
    unsigned char out[64];
    LockObject(out);
