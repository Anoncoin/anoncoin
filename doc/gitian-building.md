...
Gitian is the deterministic build process that is used to build the Anoncoin
...
- [Building Anoncoin](#building-anoncoin)
...
Clone the git repositories for anoncoin and gitian,
...
In [doc/release-process.md](release-process.md) in the anoncoin repository under 'Fetch and build inputs'.
...
To build Anoncoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the anoncoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/anoncoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/Anoncoin/anoncoin
...
For example:
```bash
URL=https://github.com/laanwj/anoncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit anoncoin=${COMMIT} --url anoncoin=${URL} ../anoncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit anoncoin=${COMMIT} --url anoncoin=${URL} ../anoncoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/anoncoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/anoncoin-build.assert
```
...
[anoncoin/gitian.sigs](https://github.com/Anoncoin/gitian.sigs/) repository, or
